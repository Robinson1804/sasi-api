...
-- =============================================================================
-- 15 tablas + 3 vistas + datos semilla + usuarios mock
...
-- =============================================================================
-- DATOS SEMILLA — USUARIOS MOCK (alineados con el frontend)
-- =============================================================================
-- Passwords: hash de 'sasi2026' (bcrypt placeholder)
...
VALUES
    ...
    ('45678901', 'Torres', 'Ana', 'Locador',
     'Analista de Sistemas', 'atorres@inei.gob.pe', '2201',
     'Oficina Técnica de Informática',
     (SELECT id FROM sedes WHERE nombre='Lima - Sede Central'),
     '2026-01-15', '2026-06-14', 'ACTIVO'),

    ('56789012', 'Salas', 'Pedro', 'Practicante',
     'Practicante Profesional TI', 'psalas@inei.gob.pe', '2310',
     'Oficina Técnica de Informática',
     (SELECT id FROM sedes WHERE nombre='Arequipa'),
     '2026-02-01', '2026-07-31', 'ACTIVO'),

    ('67890123', 'Ríos', 'Carmen', 'Orden de Servicio',
     'Asistente Administrativo', 'crios@inei.gob.pe', NULL,
     'Gerencia de Operaciones',
     (SELECT id FROM sedes WHERE nombre='Trujillo'),
     '2025-10-01', '2026-03-31', 'INACTIVO');

-- Usuarios (login) — password placeholder: $2b$10$... (bcrypt de 'sasi2026')
...
-- Ana (45678901) → solicitante
INSERT INTO usuario_roles (id_usuario, id_rol)
SELECT u.id, r.id FROM usuarios u
JOIN personal p ON u.id_personal = p.id
CROSS JOIN roles r
WHERE p.dni = '45678901' AND r.codigo = 'usuario_solicitante';

-- Pedro (56789012) → solicitante
INSERT INTO usuario_roles (id_usuario, id_rol)
SELECT u.id, r.id FROM usuarios u
JOIN personal p ON u.id_personal = p.id
CROSS JOIN roles r
WHERE p.dni = '56789012' AND r.codigo = 'usuario_solicitante';

-- =============================================================================
-- DATOS SEMILLA — SOLICITUDES MOCK (las mismas 4 del frontend)
-- =============================================================================

-- Sol 1: SASI-2026-000008 — COMPLETADA — C1
INSERT INTO solicitudes (numero, id_solicitante, estado, tipo, snap_nombres, snap_dni, snap_cargo, snap_vinculo, snap_correo, snap_oficina, snap_sede, compromiso_aceptado, fecha_creacion, fecha_envio, fecha_cierre, pdf_url)
SELECT 'SASI-2026-000008', p.id, 'completada', 'individual',
       'Juan', '12345678', 'Analista TI', 'CAS', 'jramirez@inei.gob.pe',
       'Oficina Técnica de Informática', 'Lima - Sede Central', true,
       '2026-02-09 16:00:00', '2026-02-10 07:55:00', '2026-02-12 14:00:00',
       '/mock/SASI-2026-000008.pdf'
FROM personal p WHERE p.dni = '12345678';

INSERT INTO solicitud_servicios (id_solicitud, id_servicio, estado, datos)
SELECT s.id, sv.id, 'atendido',
       '{"tipoOperacion":"creacion","redSolicitar":true,"redTipoCuenta":"personal","internetSolicitar":true,"internetPerfil":"3","correoSolicitar":true,"correoTipo":"creacion"}'::jsonb
FROM solicitudes s, servicios sv
WHERE s.numero = 'SASI-2026-000008' AND sv.codigo = 'c1';

-- Sol 2: SASI-2026-000009 — EN_PROCESO — C4 + C7
INSERT INTO solicitudes (numero, id_solicitante, estado, tipo, snap_nombres, snap_dni, snap_cargo, snap_vinculo, snap_correo, snap_oficina, snap_sede, compromiso_aceptado, fecha_creacion, fecha_envio, pdf_url)
SELECT 'SASI-2026-000009', p.id, 'en_proceso', 'individual',
       'Juan', '12345678', 'Analista TI', 'CAS', 'jramirez@inei.gob.pe',
       'Oficina Técnica de Informática', 'Lima - Sede Central', true,
       '2026-03-04 14:00:00', '2026-03-05 07:50:00',
       '/mock/SASI-2026-000009.pdf'
FROM personal p WHERE p.dni = '12345678';

INSERT INTO solicitud_servicios (id_solicitud, id_servicio, estado, datos)
SELECT s.id, sv.id, 'en_revision',
       '{"usuarioRed":"jramirez","correoPersonal":"juan.ramirez@gmail.com","nombreHost":"PC-OTIN-045","telefonoContacto":"987654321","fechaInicio":"2026-03-10","fechaTermino":"2026-03-31","justificacion":"Trabajo remoto por comisión de servicios"}'::jsonb
FROM solicitudes s, servicios sv
WHERE s.numero = 'SASI-2026-000009' AND sv.codigo = 'c4';

INSERT INTO solicitud_servicios (id_solicitud, id_servicio, estado, datos)
SELECT s.id, sv.id, 'pendiente',
       '{"tipoSolicitud":"acceso","servidor":"\\\\SVRINEI","carpeta":"\\\\SVRINEI\\OTIN\\PROYECTOS","nivelPermiso":"escritura","justificacion":"Acceso a carpeta de proyectos OTIN"}'::jsonb
FROM solicitudes s, servicios sv
WHERE s.numero = 'SASI-2026-000009' AND sv.codigo = 'c7';

-- Sol 3: SASI-2026-000010 — ENVIADA — C8 + C9
INSERT INTO solicitudes (numero, id_solicitante, estado, tipo, snap_nombres, snap_dni, snap_cargo, snap_vinculo, snap_correo, snap_oficina, snap_sede, compromiso_aceptado, fecha_creacion, fecha_envio, pdf_url)
SELECT 'SASI-2026-000010', p.id, 'enviada', 'individual',
       'Juan', '12345678', 'Analista TI', 'CAS', 'jramirez@inei.gob.pe',
       'Oficina Técnica de Informática', 'Lima - Sede Central', true,
       '2026-03-09 14:30:00', '2026-03-09 14:55:00',
       '/mock/SASI-2026-000010.pdf'
FROM personal p WHERE p.dni = '12345678';

INSERT INTO solicitud_servicios (id_solicitud, id_servicio, estado, datos)
SELECT s.id, sv.id, 'pendiente',
       '{"servidor":"dbserver01.inei.gob.pe","nombreBD":"BD_CENSOS_PROD","ambiente":"produccion","tipoAcceso":"permanente","permisoLectura":true,"permisoEscritura":false,"permisoEjecucion":false,"permisoDDL":false,"objetosEspecificos":"","justificacion":"Análisis de datos censales para proyecto OTIN-2026-003"}'::jsonb
FROM solicitudes s, servicios sv
WHERE s.numero = 'SASI-2026-000010' AND sv.codigo = 'c8';

INSERT INTO solicitud_servicios (id_solicitud, id_servicio, estado, datos)
SELECT s.id, sv.id, 'pendiente',
       '{"nombreSistema":"SIGE","modulo":"Consultas","fechaAlta":"2026-03-15","fechaBaja":"2026-12-31","tipoAcceso":"consulta","sustento":"Consulta de datos estadísticos para informes OTIN","usuarios":[]}'::jsonb
FROM solicitudes s, servicios sv
WHERE s.numero = 'SASI-2026-000010' AND sv.codigo = 'c9';

-- Sol 4: SASI-2026-000011 — EN_PROCESO — MASIVA — C1 + C4
INSERT INTO solicitudes (numero, id_solicitante, estado, tipo, snap_nombres, snap_dni, snap_cargo, snap_vinculo, snap_correo, snap_oficina, snap_sede, compromiso_aceptado, fecha_creacion, fecha_envio, pdf_url)
SELECT 'SASI-2026-000011', p.id, 'en_proceso', 'masiva',
       'Roberto', '99999999', 'Jefe OTIN', 'Nombrado', 'rcastro@inei.gob.pe',
       'Oficina Técnica de Informática', 'Lima - Sede Central', true,
       '2026-03-07 16:00:00', '2026-03-08 07:45:00',
       '/mock/SASI-2026-000011.pdf'
FROM personal p WHERE p.dni = '99999999';

INSERT INTO solicitud_servicios (id_solicitud, id_servicio, estado, datos)
SELECT s.id, sv.id, 'en_revision',
       '{"tipoOperacion":"creacion","justificacion":"Cuentas de red e internet para nuevos integrantes del equipo censo 2026"}'::jsonb
FROM solicitudes s, servicios sv
WHERE s.numero = 'SASI-2026-000011' AND sv.codigo = 'c1';

INSERT INTO solicitud_servicios (id_solicitud, id_servicio, estado, datos)
SELECT s.id, sv.id, 'pendiente',
       '{"justificacion":"Acceso VPN requerido para trabajo remoto del equipo de campo"}'::jsonb
FROM solicitudes s, servicios sv
WHERE s.numero = 'SASI-2026-000011' AND sv.codigo = 'c4';

-- Usuarios masivos de Sol 4
INSERT INTO usuarios_masivos (id_solicitud, dni, nombres, apellidos, cargo, internet_perfil, correo_personal, telefono_contacto, nombre_host)
SELECT s.id, '45678901', 'Ana', 'Torres', 'Analista de Sistemas', '3', 'ana.torres@gmail.com', '912345678', 'LAPTOP-TORRES'
FROM solicitudes s WHERE s.numero = 'SASI-2026-000011';

INSERT INTO usuarios_masivos (id_solicitud, dni, nombres, apellidos, cargo, internet_perfil, correo_personal, telefono_contacto, nombre_host)
SELECT s.id, '56789012', 'Pedro', 'Salas', 'Técnico Informático', '2', 'pedro.salas@gmail.com', '923456789', 'PC-SALAS-01'
FROM solicitudes s WHERE s.numero = 'SASI-2026-000011';

INSERT INTO usuarios_masivos (id_solicitud, dni, nombres, apellidos, cargo, internet_perfil, correo_personal, telefono_contacto, nombre_host)
SELECT s.id, '67890123', 'Carmen', 'Ríos', 'Asistente Administrativo', '3', 'carmen.rios@gmail.com', '934567890', 'PC-RIOS-01'
FROM solicitudes s WHERE s.numero = 'SASI-2026-000011';

-- =============================================================================
-- DATOS SEMILLA — ETAPAS DE APROBACIÓN (alineadas con frontend)
-- =============================================================================

-- Sol 1 (completada): Jefe→aprobado, Seguridad→aprobado, Redes→atendido
INSERT INTO etapas_aprobacion (id_solicitud_servicio, orden, id_rol, estado, comentario, fecha_inicio, fecha_accion, horas_transcurridas, sla_horas, vencio_sla)
SELECT ss.id, 1, r.id, 'aprobado', 'Aprobado conforme.',
       '2026-02-10 08:00:00', '2026-02-10 10:30:00', 2.5, 24, false
FROM solicitud_servicios ss
JOIN solicitudes s ON ss.id_solicitud = s.id
CROSS JOIN roles r
WHERE s.numero = 'SASI-2026-000008' AND r.codigo = 'jefe_supervisor';

INSERT INTO etapas_aprobacion (id_solicitud_servicio, orden, id_rol, id_aprobador, estado, comentario, fecha_inicio, fecha_accion, horas_transcurridas, sla_horas, vencio_sla)
SELECT ss.id, 2, r.id,
       (SELECT u.id FROM usuarios u JOIN personal p ON u.id_personal = p.id WHERE p.dni = '88888888'),
       'aprobado', 'Revisado y aprobado.',
       '2026-02-10 10:30:00', '2026-02-11 09:00:00', 22.5, 24, false
FROM solicitud_servicios ss
JOIN solicitudes s ON ss.id_solicitud = s.id
CROSS JOIN roles r
WHERE s.numero = 'SASI-2026-000008' AND r.codigo = 'seguridad_accesos';

INSERT INTO etapas_aprobacion (id_solicitud_servicio, orden, id_rol, estado, comentario, fecha_inicio, fecha_accion, horas_transcurridas, sla_horas, vencio_sla)
SELECT ss.id, 3, r.id, 'atendido', 'Cuenta creada y habilitada exitosamente.',
       '2026-02-11 09:00:00', '2026-02-12 14:00:00', 29, 8, false
FROM solicitud_servicios ss
JOIN solicitudes s ON ss.id_solicitud = s.id
CROSS JOIN roles r
WHERE s.numero = 'SASI-2026-000008' AND r.codigo = 'equipo_redes';

-- Sol 2 (en_proceso): Jefe→aprobado, Seguridad→en_revision, Redes→pendiente
-- Para C4:
INSERT INTO etapas_aprobacion (id_solicitud_servicio, orden, id_rol, estado, comentario, fecha_inicio, fecha_accion, horas_transcurridas, sla_horas, vencio_sla)
SELECT ss.id, 1, r.id, 'aprobado', 'Autorizado el acceso VPN por trabajo remoto.',
       '2026-03-05 08:00:00', '2026-03-05 11:00:00', 3, 24, false
FROM solicitud_servicios ss
JOIN solicitudes s ON ss.id_solicitud = s.id
JOIN servicios sv ON ss.id_servicio = sv.id
CROSS JOIN roles r
WHERE s.numero = 'SASI-2026-000009' AND sv.codigo = 'c4' AND r.codigo = 'jefe_supervisor';

INSERT INTO etapas_aprobacion (id_solicitud_servicio, orden, id_rol, id_aprobador, estado, fecha_inicio, horas_transcurridas, sla_horas, vencio_sla)
SELECT ss.id, 2, r.id,
       (SELECT u.id FROM usuarios u JOIN personal p ON u.id_personal = p.id WHERE p.dni = '88888888'),
       'en_revision',
       '2026-03-05 11:00:00', 18, 24, false
FROM solicitud_servicios ss
JOIN solicitudes s ON ss.id_solicitud = s.id
JOIN servicios sv ON ss.id_servicio = sv.id
CROSS JOIN roles r
WHERE s.numero = 'SASI-2026-000009' AND sv.codigo = 'c4' AND r.codigo = 'seguridad_accesos';

INSERT INTO etapas_aprobacion (id_solicitud_servicio, orden, id_rol, estado, fecha_inicio, sla_horas, vencio_sla)
SELECT ss.id, 3, r.id, 'pendiente', '2026-03-05 11:00:00', 8, false
FROM solicitud_servicios ss
JOIN solicitudes s ON ss.id_solicitud = s.id
JOIN servicios sv ON ss.id_servicio = sv.id
CROSS JOIN roles r
WHERE s.numero = 'SASI-2026-000009' AND sv.codigo = 'c4' AND r.codigo = 'equipo_redes';

-- Sol 3 (enviada): todas las etapas pendiente
INSERT INTO etapas_aprobacion (id_solicitud_servicio, orden, id_rol, estado, fecha_inicio, sla_horas, vencio_sla)
SELECT ss.id, 1, r.id, 'pendiente', '2026-03-09 15:00:00', 24, false
FROM solicitud_servicios ss
JOIN solicitudes s ON ss.id_solicitud = s.id
JOIN servicios sv ON ss.id_servicio = sv.id
CROSS JOIN roles r
WHERE s.numero = 'SASI-2026-000010' AND sv.codigo = 'c8' AND r.codigo = 'jefe_supervisor';

INSERT INTO etapas_aprobacion (id_solicitud_servicio, orden, id_rol, estado, fecha_inicio, sla_horas, vencio_sla)
SELECT ss.id, 2, r.id, 'pendiente', '2026-03-09 15:00:00', 48, false
FROM solicitud_servicios ss
JOIN solicitudes s ON ss.id_solicitud = s.id
JOIN servicios sv ON ss.id_servicio = sv.id
CROSS JOIN roles r
WHERE s.numero = 'SASI-2026-000010' AND sv.codigo = 'c8' AND r.codigo = 'dba';

INSERT INTO etapas_aprobacion (id_solicitud_servicio, orden, id_rol, estado, fecha_inicio, sla_horas, vencio_sla)
SELECT ss.id, 1, r.id, 'pendiente', '2026-03-09 15:00:00', 4, false
FROM solicitud_servicios ss
JOIN solicitudes s ON ss.id_solicitud = s.id
JOIN servicios sv ON ss.id_servicio = sv.id
CROSS JOIN roles r
WHERE s.numero = 'SASI-2026-000010' AND sv.codigo = 'c9' AND r.codigo = 'soporte_tecnico';

-- Sol 4 (masiva en_proceso): Coord→aprobado, Seguridad→en_revision, Redes→pendiente
INSERT INTO etapas_aprobacion (id_solicitud_servicio, orden, id_rol, estado, comentario, fecha_inicio, fecha_accion, horas_transcurridas, sla_horas, vencio_sla)
SELECT ss.id, 1, r.id, 'aprobado', 'Aprobado para los 3 nuevos integrantes del equipo.',
       '2026-03-08 08:00:00', '2026-03-08 09:30:00', 1.5, 24, false
FROM solicitud_servicios ss
JOIN solicitudes s ON ss.id_solicitud = s.id
JOIN servicios sv ON ss.id_servicio = sv.id
CROSS JOIN roles r
WHERE s.numero = 'SASI-2026-000011' AND sv.codigo = 'c1' AND r.codigo = 'coordinador_proyecto';

INSERT INTO etapas_aprobacion (id_solicitud_servicio, orden, id_rol, id_aprobador, estado, fecha_inicio, horas_transcurridas, sla_horas, vencio_sla)
SELECT ss.id, 2, r.id,
       (SELECT u.id FROM usuarios u JOIN personal p ON u.id_personal = p.id WHERE p.dni = '88888888'),
       'en_revision',
       '2026-03-08 09:30:00', 12, 24, false
FROM solicitud_servicios ss
JOIN solicitudes s ON ss.id_solicitud = s.id
JOIN servicios sv ON ss.id_servicio = sv.id
CROSS JOIN roles r
WHERE s.numero = 'SASI-2026-000011' AND sv.codigo = 'c1' AND r.codigo = 'seguridad_accesos';

INSERT INTO etapas_aprobacion (id_solicitud_servicio, orden, id_rol, estado, fecha_inicio, sla_horas, vencio_sla)
SELECT ss.id, 3, r.id, 'pendiente', '2026-03-08 09:30:00', 8, false
FROM solicitud_servicios ss
JOIN solicitudes s ON ss.id_solicitud = s.id
JOIN servicios sv ON ss.id_servicio = sv.id
CROSS JOIN roles r
WHERE s.numero = 'SASI-2026-000011' AND sv.codigo = 'c1' AND r.codigo = 'equipo_redes';

-- =============================================================================
-- DATOS SEMILLA — HISTORIAL DE EVENTOS
-- =============================================================================

-- Sol 1 historial
INSERT INTO historial (id_solicitud, tipo_evento, estado_nuevo, comentario, usuario_nombre, created_at)
SELECT s.id, 'CREACION', 'borrador', 'Solicitud creada.', 'Juan Ramírez', '2026-02-09 16:00:00'
FROM solicitudes s WHERE s.numero = 'SASI-2026-000008';
INSERT INTO historial (id_solicitud, tipo_evento, estado_nuevo, comentario, usuario_nombre, created_at)
SELECT s.id, 'ENVIO', 'enviada', 'Solicitud enviada para aprobación.', 'Juan Ramírez', '2026-02-10 07:55:00'
FROM solicitudes s WHERE s.numero = 'SASI-2026-000008';
INSERT INTO historial (id_solicitud, tipo_evento, estado_nuevo, comentario, usuario_nombre, created_at)
SELECT s.id, 'APROBACION', 'en_proceso', 'Aprobado por Jefe inmediato.', 'Carlos Mendoza', '2026-02-10 10:30:00'
FROM solicitudes s WHERE s.numero = 'SASI-2026-000008';
INSERT INTO historial (id_solicitud, tipo_evento, estado_nuevo, comentario, usuario_nombre, created_at)
SELECT s.id, 'APROBACION', 'en_proceso', 'Revisado y aprobado por Seguridad.', 'Mario Ávila', '2026-02-11 09:00:00'
FROM solicitudes s WHERE s.numero = 'SASI-2026-000008';
INSERT INTO historial (id_solicitud, tipo_evento, estado_nuevo, comentario, usuario_nombre, created_at)
SELECT s.id, 'ATENCION', 'completada', 'Cuenta de red, internet y correo creados exitosamente.', 'Luis Paredes', '2026-02-12 14:00:00'
FROM solicitudes s WHERE s.numero = 'SASI-2026-000008';

-- Sol 2 historial
INSERT INTO historial (id_solicitud, tipo_evento, estado_nuevo, comentario, usuario_nombre, created_at)
SELECT s.id, 'CREACION', 'borrador', 'Solicitud creada.', 'Juan Ramírez', '2026-03-04 14:00:00'
FROM solicitudes s WHERE s.numero = 'SASI-2026-000009';
INSERT INTO historial (id_solicitud, tipo_evento, estado_nuevo, comentario, usuario_nombre, created_at)
SELECT s.id, 'ENVIO', 'enviada', 'Solicitud enviada para aprobación.', 'Juan Ramírez', '2026-03-05 07:50:00'
FROM solicitudes s WHERE s.numero = 'SASI-2026-000009';
INSERT INTO historial (id_solicitud, tipo_evento, estado_nuevo, comentario, usuario_nombre, created_at)
SELECT s.id, 'APROBACION', 'en_proceso', 'Aprobado por Jefe inmediato.', 'Carlos Mendoza', '2026-03-05 11:00:00'
FROM solicitudes s WHERE s.numero = 'SASI-2026-000009';

-- Sol 3 historial
INSERT INTO historial (id_solicitud, tipo_evento, estado_nuevo, comentario, usuario_nombre, created_at)
SELECT s.id, 'CREACION', 'borrador', 'Solicitud creada.', 'Juan Ramírez', '2026-03-09 14:30:00'
FROM solicitudes s WHERE s.numero = 'SASI-2026-000010';
INSERT INTO historial (id_solicitud, tipo_evento, estado_nuevo, comentario, usuario_nombre, created_at)
SELECT s.id, 'ENVIO', 'enviada', 'Solicitud enviada para aprobación.', 'Juan Ramírez', '2026-03-09 14:55:00'
FROM solicitudes s WHERE s.numero = 'SASI-2026-000010';

-- Sol 4 historial (masiva)
INSERT INTO historial (id_solicitud, tipo_evento, estado_nuevo, comentario, usuario_nombre, created_at)
SELECT s.id, 'CREACION', 'borrador', 'Solicitud masiva creada para 3 usuarios.', 'Roberto Castro', '2026-03-07 16:00:00'
FROM solicitudes s WHERE s.numero = 'SASI-2026-000011';
INSERT INTO historial (id_solicitud, tipo_evento, estado_nuevo, comentario, usuario_nombre, created_at)
SELECT s.id, 'ENVIO', 'enviada', 'Solicitud masiva enviada para aprobación.', 'Roberto Castro', '2026-03-08 07:45:00'
FROM solicitudes s WHERE s.numero = 'SASI-2026-000011';
INSERT INTO historial (id_solicitud, tipo_evento, estado_nuevo, comentario, usuario_nombre, created_at)
SELECT s.id, 'APROBACION', 'en_proceso', 'Aprobado por Coordinador de Proyecto.', 'Ana García', '2026-03-08 09:30:00'
FROM solicitudes s WHERE s.numero = 'SASI-2026-000011';
