...
async function listar(req, res) {
  try {
    ...
    // Allow fetching all for admin listing (max 5000)
...
async function crear(req, res) {
  try {
    const {
      dni, apellidos, nombres, tipoVinculo, cargo,
      correo, telefono, oficina, sede, roles, activo,
    } = req.body;
...
async function actualizar(req, res) {
  try {
    ...
    const result = await queries.actualizar(id, {
      cargo, oficina, sede, roles, activo,
    });
...
async function sincronizar(req, res) {
  try {
    // Obtener TODOS los registros del Excel (sin paginación)
    ...
    // Mapear campos del Excel al formato esperado por queries.sincronizar
    const listaExternos = lista.map((ext) => ({
      dni:         ext.dni,
      apellidos:   ext.apellidos,
      nombres:     ext.nombres,
      tipoVinculo: VINCULO_MAP[ext.tipoVinculo] || ext.tipoVinculo,
      cargo:       ext.cargo,
      correo:      ext.correo,
      celular:     ext.celular,
      unidad:      ext.unidad,
